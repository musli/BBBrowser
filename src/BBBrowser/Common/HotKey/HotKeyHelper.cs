...
namespace Common.HotKey
{
    public class HotKeyHelper
    {
        ...
        /// 记录快捷键注册项的唯一标识符
        /// </summary>
        private static Dictionary<BBHotKey, int> m_HotKeySettingsDic = new Dictionary<BBHotKey, int>();
        ...
        public static string RegisterGlobalHotKey(IEnumerable<HotKeyEntity> hotKeyModelList, IntPtr hwnd, out Dictionary<BBHotKey, int> hotKeySettingsDic)
        {
        ...
        private static bool RegisterHotKey(HotKeyEntity hotKeyModel, IntPtr hWnd)
        {
            var fsModifierKey = new ModifierKeys();
            var hotKeySetting = (BBHotKey)Enum.Parse(typeof(BBHotKey), hotKeyModel.Name);

            if (!m_HotKeySettingsDic.ContainsKey(hotKeySetting))
            {
                // 全局原子不会在应用程序终止时自动删除。每次调用GlobalAddAtom函数，必须相应的调用GlobalDeleteAtom函数删除原子。
                if (Kernel.GlobalFindAtom(hotKeySetting.ToString()) != 0)
                {
                    Kernel.GlobalDeleteAtom(Kernel.GlobalFindAtom(hotKeySetting.ToString()));
                }
                // 获取唯一标识符
                m_HotKeySettingsDic[hotKeySetting] = Kernel.GlobalAddAtom(hotKeySetting.ToString());
            }
            else
            {
                // 注销旧的热键
                User.UnregisterHotKey(hWnd, m_HotKeySettingsDic[hotKeySetting]);
            }
            ...
            return Convert.ToBoolean(User.RegisterHotKey(hWnd, m_HotKeySettingsDic[hotKeySetting], (int)fsModifierKey, (int)hotKeyModel.SelectKey));
        }
